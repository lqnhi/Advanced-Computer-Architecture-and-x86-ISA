...
.program 0
    ...
    add R1, 0, 64        -- R1 = base address of matrix b (address 256)
    add R2, 0, 128        -- R2 = base address of matrix c (address 512)
...
.L3
    ...
    mult R9, R6, SIZE     -- R9 = k * SIZE (row offset in B)
    add R9, R9, R4        -- R9 = k * SIZE + j (element offset in B)
    add R9, R1, R9        -- R9 = base_b + (k * SIZE + j)
    li R10, (R9)          -- R10 = b[k][j]
    nop                   -- Avoid hazard after load
    
    mult R11, R8, R10     -- R11 = a[i][k] * b[k][j]
    add R5, R5, R11       -- sum += a[i][k] * b[k][j]
    
    add R6, R6, 1         -- k++
    sub R12, R6, SIZE     -- R12 = k - SIZE
    brnz R12, L3          -- If k < SIZE, continue inner loop
    
    mult R13, R3, SIZE    -- R13 = i * SIZE (row offset in C)
    add R13, R13, R4      -- R13 = i * SIZE + j (element offset in C)
    add R13, R2, R13      -- R13 = base_c + (i * SIZE + j)
    si (R13), R5          -- c[i][j] = sum
